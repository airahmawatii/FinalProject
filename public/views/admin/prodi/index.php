    ...
    <main class="flex-1 min-h-screen relative">
        ...
        <div class="p-6 md:p-10 max-w-7xl mx-auto pt-20 md:pt-10 relative z-10">
            ...
            <header class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                <div>
                     <h1 class="text-3xl md:text-4xl font-bold mb-2 text-white">Program Studi</h1>
                     <p class="text-blue-200">Manajemen jurusan dan fakultas dalam sistem akademik.</p>
                </div>
            ...
            <?php if (isset($_GET['msg']) && $_GET['msg'] == 'deleted'): ?>
                <div class="glass border-green-500/30 text-green-300 p-4 rounded-xl mb-6">Prodi berhasil dihapus!</div>
            <?php endif; ?>

            <!-- Action Bar -->
            <div class="flex justify-end mb-8">
                <a href="prodi_add.php" class="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4 rounded-2xl text-sm font-bold text-white shadow-lg shadow-blue-500/20 hover:scale-105 transition flex items-center gap-2 border border-white/10">
                ...
                <table class="w-full text-left">
                    <thead class="bg-gradient-to-r from-white/5 to-transparent border-b border-white/10">
                        <tr>
                            ...
                            <th class="p-6 text-blue-300 font-bold uppercase tracking-wider text-[10px]">Kode Institusi</th>
                            <th class="p-6 text-blue-300 font-bold uppercase tracking-wider text-[10px]">Nama Program Studi</th>
                        ...
                        <?php if (!empty($rows)): ?>
                            ...
                            <tr class="hover:bg-white/[0.03] transition-colors group">
                                <td class="p-6 font-mono text-[10px] text-blue-400 font-bold">
                                    <span class="bg-blue-500/10 px-2 py-1 rounded-lg border border-blue-500/20">#<?= $r['id_prodi'] ?></span>
                                </td>
                                <td class="p-6">
                                    <span class="bg-indigo-500/10 text-indigo-300 px-3 py-1.5 rounded-xl text-[10px] font-extrabold uppercase tracking-widest border border-indigo-500/20">
                                        <?= htmlspecialchars($r['kode_prodi']) ?>
                                    </span>
                                </td>
                                <td class="p-6 font-bold text-white text-base group-hover:text-blue-400 transition"><?= htmlspecialchars($r['nama_prodi']) ?></td>
                                <td class="p-6 text-right">
                                    <div class="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                        <a href="prodi_edit.php?id=<?= $r['id_prodi'] ?>" class="glass bg-blue-500/10 hover:bg-blue-500/20 text-blue-300 p-2.5 rounded-xl transition border border-blue-500/30">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                        </a>
                                        <a href="prodi_delete.php?id=<?= $r['id_prodi'] ?>" onclick="return confirm('Yakin?')" class="glass bg-red-500/10 hover:bg-red-500/20 text-red-300 p-2.5 rounded-xl transition border border-red-500/30">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="4" class="p-20 text-center text-slate-500 italic">
                                    ...
                                    <p class="font-medium">Belum ada data prodi ditemukan.</p>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
