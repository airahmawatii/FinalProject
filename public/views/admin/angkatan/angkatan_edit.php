...
$data = $model->find($id);
if (!$data) die("Angkatan tidak ditemukan");

$error = "";

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($model->update($id, $_POST['tahun'])) {
        header("Location: index.php?msg=updated");
        exit;
    } else {
        $error = "Gagal mengupdate angkatan.";
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Edit Angkatan | Admin</title>
...
<body class="bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 min-h-screen p-8 flex items-center justify-center font-outfit">
    ...
    <div class="w-full max-w-xl glass p-10 rounded-[2.5rem] shadow-2xl relative z-10 border border-white/20 overflow-hidden">
        ...
        <div class="flex justify-between items-center mb-10">
            <div>
                <h2 class="text-3xl font-bold text-white tracking-tight">Edit Angkatan</h2>
                <p class="text-blue-300/60 text-sm mt-1">Perbarui tahun angkatan akademik mahasiswa.</p>
            </div>
        ...
        <form method="POST" class="space-y-6">
            <div class="space-y-2">
                <label class="block text-[10px] font-extrabold text-blue-300 uppercase tracking-widest ml-1">Tahun Angkatan Akademik</label>
                <input type="number" name="tahun" required placeholder="Contoh: 2024" min="2000" max="2100"
                       value="<?= htmlspecialchars($data['tahun']) ?>"
                       class="w-full px-5 py-4 glass rounded-2xl focus:ring-4 focus:ring-blue-500/20 focus:outline-none text-white border-white/10 transition-all text-xl font-bold">
            </div>

            <div class="flex gap-4 pt-6">
                <button type="submit" class="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white py-4 rounded-2xl font-bold shadow-xl shadow-blue-500/20 transition-all hover:scale-[1.02] active:scale-[0.98] border border-white/10 text-lg">
                    Update Angkatan
                </button>
